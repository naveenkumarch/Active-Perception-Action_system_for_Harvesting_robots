...
fnameWithPath1 = "/content/yolov5/Texture_classification"
...
def run(weights='yolov5s.pt',  # model.pt path(s)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            resz_img = cv2.resize(imc,(100,100),interpolation = cv2.INTER_AREA)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    volume = (math.pi * math.pow((xywh[2]/2),2) *xywh[3])/3
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        #Printing label on the saved image 
                        plable = "Mp: "+names[c]+" Tp: "+Tlable + " Vol :"+ str(round(volume,2))
                        mlable = str(xywh)  
                        label = None if hide_labels else (plable if hide_conf else f'{plable} {conf:.2f}')
                        plot_one_box(xyxy, im0, label=label, color=colors(c, True), line_thickness=line_thickness)
                        #plot_one_box(xywh, im0, label=mlable, color=colors(c, True), line_thickness=line_thickness)
                        if save_crop:
